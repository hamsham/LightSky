...
-------------------------------------*/
sceneNode meshResource::createPrimitiveNode(const char * const name) {
    sceneNode primNode;

    primNode.pParent = &primNode;
    primNode.name = name;
    primNode.meshIndices.clear();
    primNode.childList.clear();
    primNode.nodeTransform.setTransform(math::mat4{1.f});

    draw_index_pair_t indexPair;
    indexPair.first = 0;
    indexPair.count = vertexList.size();
    primNode.meshIndices.push_back(indexPair);
